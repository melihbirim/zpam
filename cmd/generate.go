...
Example:
  zpam generate plugin my_awesome_plugin`,
...
func runGeneratePlugin(cmd *cobra.Command, args []string) {
	...
	tmpl := &PluginTemplate{
		...
		Author:       pluginAuthor,
		Description:  fmt.Sprintf("%s plugin for ZPAM spam filter", strings.Title(strings.ReplaceAll(pluginName, "_", " "))),
	}
	...
	fmt.Printf("   ./zpam plugins test-one %s examples/test_headers.eml\n", pluginName)
}
...
// Generated by ZPAM plugin generator
...
` + "```bash" + `
./zpam plugins test-one {{.Name}} examples/test_headers.eml
` + "```" + `
...
Enable the plugin in your production configuration and restart ZPAM.
