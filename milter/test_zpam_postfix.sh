...
# 🫏 ZPAM + Postfix Integration Test Script
...
MILTER_PORT="7357"
POSTFIX_TEST_CONFIG="/tmp/postfix_main.cf.zpam_test"
...
check_prerequisites() {
    ...
    # Check if ZPAM binary exists
    if [[ ! -f "$ZPAM_BINARY" ]]; then
        error "ZPAM binary not found at $ZPAM_BINARY. Run 'go build -o zpam .' first."
    fi
...
setup_environment() {
    ...
    if [[ -f "/etc/postfix/main.cf" ]]; then
        sudo cp /etc/postfix/main.cf /etc/postfix/main.cf.backup.zpam || warning "Could not backup postfix config"
    fi
    
    # Create test postfix configuration
    cat > "$POSTFIX_TEST_CONFIG" << 'EOF'
# ZPAM + Postfix Integration Test Configuration
...
# Start ZPAM milter server
start_zpam_milter() {
    log "Starting ZPAM milter server..."
    
    # Kill any existing ZPAM processes
    pkill -f "zpam milter" 2>/dev/null || true
    
    # Start ZPAM milter in background  
    $ZPAM_BINARY milter --config "$CONFIG_FILE" --debug > zpam_milter.log 2>&1 &
    ZPAM_PID=$!
    ...
    if ! kill -0 $ZPAM_PID 2>/dev/null; then
        error "Failed to start ZPAM milter. Check zpam_milter.log for details."
    fi
    
    # Test connectivity
    if ! nc -z 127.0.0.1 $MILTER_PORT 2>/dev/null; then
        error "ZPAM milter is not listening on port $MILTER_PORT"
    fi
    
    success "ZPAM milter server started (PID: $ZPAM_PID)"
}
...
setup_postfix() {
    ...
    success "Postfix configured and started with ZPAM milter integration"
}
...
send_test_emails() {
    ...
    echo "🫏 ZPAM Email Test Suite - 10 Emails (5 Clean, 5 Spam)"
    echo "====================================================="
    ...
    echo
    echo "🔍 Check /var/mail/$USER for X-ZPAM-* headers"
}
...
check_test_results() {
    ...
    # Count X-ZPAM headers
    zpam_headers=$(grep -c "X-ZPAM-Status:" "$MAIL_FILE" 2>/dev/null || echo "0")
    
    # Check score distribution
    clean_count=$(grep -c "X-ZPAM-Status: Clean" "$MAIL_FILE" 2>/dev/null || echo "0")
    spam_count=$(grep -c "X-ZPAM-Status: Spam" "$MAIL_FILE" 2>/dev/null || echo "0")
    
    echo
    echo "📊 Test Results Summary:"
    echo "========================"
    echo "📧 Emails received: $email_count/10"
    echo "🏷️  ZPAM headers added: $zpam_headers/10"
    echo "✅ Clean emails: $clean_count"
    echo "🚨 Spam emails: $spam_count"
    
    if [[ $email_count -eq 10 && $zpam_headers -eq 10 ]]; then
        success "All emails processed successfully with ZPAM headers!"
    elif [[ $email_count -eq 10 ]]; then
        warning "All emails received but missing ZPAM headers ($zpam_headers/10)"
    else
    ...
    echo "============================"
    grep -A2 -B1 "X-ZPAM-Status:" "$MAIL_FILE" 2>/dev/null | grep -E "(Subject:|X-ZPAM-Status:|X-ZPAM-Score:)" || warning "No detailed scores found"
}
...
cleanup() {
    ...
    # Stop ZPAM milter
    if [[ -n "$ZPAM_PID" ]]; then
        kill $ZPAM_PID 2>/dev/null || true
        success "Stopped ZPAM milter"
    fi
    ...
    if [[ -f "/etc/postfix/main.cf.backup.zpam" ]]; then
        sudo cp /etc/postfix/main.cf.backup.zpam /etc/postfix/main.cf
        info "Restored original Postfix configuration"
    fi
    
    # Restore original ZPAM config if backup exists
    if [[ -f "$CONFIG_FILE.bak" ]]; then
        mv "$CONFIG_FILE.bak" "$CONFIG_FILE"
        info "Restored original ZPAM configuration"
    fi
    
    # Clean up temporary files
    rm -f "$POSTFIX_TEST_CONFIG"
    rm -f zpam_milter.log
...
# Main execution
main() {
    echo "🫏 ZPAM + Postfix Integration Test Suite"
    echo "========================================"
    ...
    info "View ZPAM logs: tail -f milter/zpam_milter.log"
