#!/bin/bash
# ZPAM Comprehensive Benchmark Script
...
BENCHMARK_FILE="${RESULTS_DIR}/zpam_benchmark_${TIMESTAMP}.md"
...
# ZPAM Benchmark Results - $TIMESTAMP
...
run_benchmark() {
    ...
    local temp_dir="/tmp/zpam_bench_${backend}_${batch_size}"
    mkdir -p "$temp_dir"
    ...
    local result
    if ! result=$(timeout 300s ./zpam filter --config "$config_file" --input "$temp_dir" $extra_flags 2>&1); then
...
# Start benchmarking
log "Starting ZPAM Comprehensive Benchmark"
...
for backend in "${BACKENDS[@]}"; do
    for batch_size in "${BATCH_SIZES[@]}"; do
        ...
        if docker exec zpam-zpam-1 bash -c "$(declare -f run_benchmark); run_benchmark '$backend' '$batch_size' '$test_name' '$config_file'"; then
...
Testing load distribution across multiple ZPAM instances.
...
for batch_size in 500 1000; do
    ...
    docker exec zpam-zpam-1 bash -c "
        emails=\$(find /app/stress-test -name '*.eml' | head -$half_batch)
        temp_dir=/tmp/bench_inst1
        mkdir -p \$temp_dir
        echo \"\$emails\" | xargs -I {} cp {} \$temp_dir/
        ./zpam filter --config /app/config.yaml --input \$temp_dir
        rm -rf \$temp_dir
    " &
    
    docker exec zpam-zpam-2 bash -c "
...
redis_stats=$(docker exec zpam-redis redis-cli info memory)
redis_keycount=$(docker exec zpam-redis redis-cli --raw eval "return #redis.call('keys', 'zpam:bayes:*')" 0)
